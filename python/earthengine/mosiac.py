...
def is_image_blank_or_incomplete(image, region, x_dim, y_dim, scale=30, debug=False):
    """
    ...
    try:
        ...
        for index in range(0, 13):
            if dict_values[index] != first_value:
                if (debug == True):
                    print("band value flagged: " + str(dict_values[index]))
                is_incomplete = True;
        for index in range(3):
            if dict_values[index + 16] != first_value:
                if (debug == True):
                    print("band value flagged: " + str(dict_values[index + 16]))
                is_incomplete = True;
    ...
    print("blank check: " + str(is_blank or is_incomplete))
    return is_blank or is_incomplete

def fetch_image_with_scaling(image, region, initial_scale=10, max_attempts=10, debug=False):
    """
...
def get_before_image(yr, mo, dy, lat1, lon1, lat2, lon2, max_attempts=5, debug=False):
...
def get_after_image(yr, mo, dy, lat1, lon1, lat2, lon2, max_attempts=10, debug=False):
    center = get_center(lat1, lon1, lat2, lon2)
    ns = get_ns_km(lat1, lon1, lat2)
    ew = get_ew_km(lat1, lon1, lon2)
    size = (ns, ew)
...
def main():
    ...
    print(get_after_image(2022, 4, 29, 37.6262, -97.1930, 37.7830, -97.0820))
    print(get_before_image(2022, 4, 29, 37.6262, -97.1930, 37.7830, -97.0820))


    print(str(get_center(37.6262, -97.1930,  37.7830, -97.0820)))
